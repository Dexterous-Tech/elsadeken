...
class NetworkHelper {
  ...
  static Future<Map<String, dynamic>> getNetworkDiagnostics() async {
    ...
    try {
      ...
      diagnostics['canResolvePusher'] = await canResolveHostname(
        'ws-eu.pusherapp.com',
      );

      // Check if we can reach Pusher servers
      diagnostics['canReachPusher'] = await testConnection(
        'ws-eu.pusherapp.com',
        443,
      );
  ...
  static bool isDNSError(String errorMessage) {
    ...
    return dnsErrorPatterns.any(
      (pattern) => errorMessage.toLowerCase().contains(pattern.toLowerCase()),
    );
  }
