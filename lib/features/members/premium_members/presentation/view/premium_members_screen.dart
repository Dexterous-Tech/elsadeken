...
class _PremiumMembersViewState extends State<PremiumMembersView> {
  ...
  Widget build(BuildContext context) {
    final cubit = MembersListCubit<UsersDataModel>(
      ({int? page}) async {
        final response =
            await sl<MembersRepository>().getDistinguishedMembers(countryName: _selectedCountryName);
                            ...
                            onTap: () async {
                              final result =
                                ...
                                builder: (context) => FilterBottomSheet(
                                  selectedCountryId: null, // We'll use country name instead
                                ),
                              );
                              if (result != null) {
                                print('🔍 Premium Members Screen - Filter result: $result');
                                setState(() {
                                  _selectedCountryName = result['name'] as String?;
                                });
                                print('🔍 Premium Members Screen - Selected country name: $_selectedCountryName');
                                // Recreate cubit with country filter
                                final newCubit = MembersListCubit<UsersDataModel>(
                                  ({int? page}) async {
                                    print('🔍 Premium Members Screen - Creating new cubit with country: $_selectedCountryName');
                                    final response =
                                        await sl<MembersRepository>().getDistinguishedMembers(countryName: _selectedCountryName);
                                    return response.data ?? [];
                                  },
                                ...
                                Navigator.of(context).pushReplacement(
                                  MaterialPageRoute(
                                    builder: (_) => BlocProvider<
                                      ...
                                      child: PremiumMembersView(countryName: _selectedCountryName),
                                    ),
                                  ),
                                );
                              }
